...
export default function HomePage() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", color: '#1e293b', overflowX: 'hidden' }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div className="fade-in">
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 20, padding: '7px 16px', fontSize: 13, fontWeight: 500, color: '#475569', marginBottom: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <span style={{ width: 7, height: 7, background: '#22c55e', borderRadius: '50%', display: 'inline-block' }} />
              Create. Share. Connect.
            </div>
            <h1 style={{ fontSize: isMobile ? '36px' : 'clamp(38px, 5vw, 62px)', fontWeight: 800, lineHeight: 1.1, marginBottom: 18, color: '#0f172a' }}>
              One Profile.<br />
            ...
            <p style={{ fontSize: isMobile ? '15px' : '17px', color: '#475569', lineHeight: 1.75, marginBottom: 28, maxWidth: 470 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 16px', marginBottom: 28 }}>
              {['Digital Business Profile', 'NFC Smart Cards', 'Business Directory', 'Lead Generation'].map(f => (
                <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 500, color: '#334155' }}>
                  <div style={{ width: 20, height: 20, background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexDirection: isMobile ? 'column' : 'row' }}>
              <Link href="/dashboard/create-profile" style={{ background: 'linear-gradient(135deg, #3b82f6, #6366f1)', color: '#fff', padding: '15px 32px', borderRadius: 12, fontSize: 15, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 4px 20px rgba(59,130,246,0.35)' }}>
                Create Your Profile
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
              </Link>
              <Link href="/demo" style={{ background: '#fff', color: '#334155', padding: '15px 32px', borderRadius: 12, fontSize: 15, fontWeight: 600, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                View Demo Profile
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
              </Link>
            </div>
      ...
      {/* NFC SMART CARDS */}
      <section id="nfc-cards" style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#0f172a', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: isMobile ? '28px' : 'clamp(30px, 4vw, 48px)', fontWeight: 800, color: '#fff', marginBottom: 12 }}>Tap. Connect. Grow.</h2>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexDirection: isMobile ? 'column' : 'row' }}>
            <Link href="/nfc-cards" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', padding: '15px 36px', borderRadius: 12, fontSize: 15, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 4px 24px rgba(99,102,241,0.45)' }}>
              Order NFC Card
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </Link>
            <Link href="/nfc-cards" style={{ background: 'rgba(255,255,255,0.07)', color: '#e2e8f0', padding: '15px 36px', borderRadius: 12, fontSize: 15, fontWeight: 600, textDecoration: 'none', border: '1px solid rgba(255,255,255,0.15)', textAlign: 'center' }}>
              Learn More
            </Link>
          </div>
        </div>
      ...
      {/* WHY SMARTPROFILE */}
      <section id="features" style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 44 }}>
            ...
            <h2 style={{ fontSize: isMobile ? '26px' : 'clamp(28px, 4vw, 42px)', fontWeight: 800, color: '#0f172a' }}>Why Businesses Choose SmartProfile</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? 12 : 20 }}>
            {features.map((f, i) => (
              <div key={i} style={{ background: '#fafafa', border: '1px solid #f1f5f9', borderRadius: 18, padding: isMobile ? '20px 14px' : '28px 20px', textAlign: 'center' }}>
                <div style={{ width: 52, height: 52, background: f.bg, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', color: f.color }}>{f.svg}</div>
                <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 7, color: '#0f172a' }}>{f.title}</div>
      ...
      {/* STATS */}
      <section ref={statsRef} style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #3b82f6 50%, #6366f1 100%)', padding: isMobile ? '52px 16px' : '72px 24px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? 20 : 24, textAlign: 'center' }}>
            {[
              ...
              { num: counts.biz, suffix: '+', label: 'Businesses Growing', icon: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="1.5"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg> },
            ].map((s, i) => (
              <div key={i} style={{ color: '#fff' }}>
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>{s.icon}</div>
                <div style={{ fontSize: isMobile ? '32px' : 'clamp(36px, 4vw, 52px)', fontWeight: 800, marginBottom: 6, lineHeight: 1 }}>
                  {s.num >= 1000 ? `${Math.floor(s.num/1000)}K` : s.num}{s.suffix}
                </div>
                <div style={{ fontSize: 13, opacity: 0.8, fontWeight: 500 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* HOW IT WORKS */}
      <section style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 44 }}>
            ...
            <h2 style={{ fontSize: isMobile ? '26px' : 'clamp(28px, 4vw, 42px)', fontWeight: 800, color: '#0f172a' }}>Create. Share. Grow.</h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? 20 : 24, position: 'relative' }}>
            {!isMobile && <div style={{ position: 'absolute', top: 36, left: '13%', right: '13%', height: 2, background: 'linear-gradient(90deg, #6366f1, #8b5cf6, #3b82f6, #10b981)', borderRadius: 2, zIndex: 0, opacity: 0.4 }} />}
            {steps.map((s, i) => (
              <div key={i} style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
                <div style={{ width: 68, height: 68, background: '#fff', border: `3px solid ${s.color}`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', boxShadow: `0 4px 20px ${s.color}30` }}>
      ...
      {/* BUSINESS DIRECTORY */}
      <section id="directory" style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#f8fafc' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 44 }}>
            ...
            <h2 style={{ fontSize: isMobile ? '26px' : 'clamp(28px, 4vw, 42px)', fontWeight: 800, color: '#0f172a' }}>Get Listed. Get Found.</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: isMobile ? 10 : 16, marginBottom: 36 }}>
            {directoryCategories.map((cat, i) => (
              <Link key={i} href="/directory" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: '16px 18px' }}>
      ...
      {/* PRICING */}
      <section id="pricing" style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 44 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#3b82f6', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Pricing</p>
            <h2 style={{ fontSize: isMobile ? '26px' : 'clamp(28px, 4vw, 42px)', fontWeight: 800, color: '#0f172a' }}>Choose Your Perfect Plan</h2>
            <p style={{ color: '#64748b', fontSize: 14, marginTop: 10 }}>All plans include QR Code + WhatsApp + Save Contact + Free Directory Listing</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4, 1fr)', gap: isMobile ? 16 : 20, alignItems: 'start' }}>
            {plans.map((p, i) => (
              <div key={i} style={{ border: p.popular ? `2px solid ${p.color}` : '1px solid #e2e8f0', borderRadius: 22, padding: '28px 20px', position: 'relative', background: p.popular ? 'linear-gradient(160deg, #eff6ff, #f5f3ff)' : '#fafafa', boxShadow: p.popular ? `0 12px 40px ${p.color}22` : 'none' }}>
                {p.popular && <div style={{ position: 'absolute', top: -13, left: '50%', transform: 'translateX(-50%)', background: `linear-gradient(135deg, ${p.color}, #8b5cf6)`, color: '#fff', fontSize: 11, fontWeight: 700, padding: '5px 16px', borderRadius: 20, whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
                ...
                <Link href="/dashboard/create-profile" style={{ display: 'block', textAlign: 'center', background: p.popular ? `linear-gradient(135deg, ${p.color}, #8b5cf6)` : 'transparent', color: p.popular ? '#fff' : p.color, border: `2px solid ${p.color}`, padding: '12px', borderRadius: 10, fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
                  Get Started
                </Link>
              </div>
            ))}
      ...
      {/* COMPARISON TABLE */}
      <section style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#f8fafc', overflowX: 'auto' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', minWidth: 320 }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            <h2 style={{ fontSize: isMobile ? '22px' : 'clamp(24px, 3vw, 38px)', fontWeight: 800, color: '#0f172a' }}>Compare All Plans</h2>
            <p style={{ color: '#64748b', marginTop: 8, fontSize: 14 }}>See exactly what you get with each plan</p>
          </div>
          <div style={{ border: '1px solid #e2e8f0', borderRadius: 18, overflow: 'hidden', background: '#fff', boxShadow: '0 4px 24px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr', background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              <div style={{ padding: '14px 14px', fontWeight: 700, fontSize: 11, color: '#64748b' }}>Feature</div>
              {['Basic','Business','Premium','Pro'].map((name,i) => (
                <div key={i} style={{ padding: '14px 6px', textAlign: 'center', fontWeight: 800, fontSize: 11, color: name==='Premium' ? '#3b82f6' : '#1e293b' }}>
                  {name}{name==='Premium' && <span style={{ display: 'block', fontSize: 9, color: '#3b82f6', fontWeight: 600, marginTop: 1 }}>★</span>}
                </div>
              ))}
            </div>
            {comparisonRows.map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr', borderBottom: i < comparisonRows.length-1 ? '1px solid #f1f5f9' : 'none', background: i%2===0 ? '#fff' : '#fafafa' }}>
                <div style={{ padding: '12px 14px', fontSize: 11, color: '#334155', fontWeight: 500 }}>{row.feature}</div>
                {[row.b, row.bu, row.p, row.pr].map((val, j) => (
                  <div key={j} style={{ padding: '12px 6px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {val
                      ? <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="10" fill="#22c55e" fillOpacity="0.15"/><path d="M6 10l3 3 5-5" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      : <svg width="16" height="16" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="10" fill="#ef4444" fillOpacity="0.1"/><path d="M7 13l6-6M13 13L7 7" stroke="#ef4444" strokeWidth="2" strokeLinecap="round"/></svg>
                    }
                  </div>
                ))}
              </div>
      ...
      <section style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#fff' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            ...
            <h2 style={{ fontSize: isMobile ? '26px' : 'clamp(28px, 4vw, 42px)', fontWeight: 800, color: '#0f172a' }}>What Our Users Say</h2>
          </div>
          <div style={{ background: 'linear-gradient(135deg, #f8fafc, #f0f4ff)', borderRadius: 22, padding: isMobile ? '24px 18px' : '44px', boxShadow: '0 4px 32px rgba(0,0,0,0.06)', border: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: 52, color: '#3b82f6', lineHeight: 0.8, marginBottom: 16, opacity: 0.2, fontFamily: 'Georgia, serif', fontWeight: 900 }}>"</div>
            <p style={{ fontSize: isMobile ? '15px' : '17px', color: '#334155', lineHeight: 1.8, marginBottom: 24, fontStyle: 'italic' }}>{testimonials[activeTestimonial].text}</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              <div style={{ width: 46, height: 46, borderRadius: '50%', background: `linear-gradient(135deg, ${testimonials[activeTestimonial].color}, #8b5cf6)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 14 }}>{testimonials[activeTestimonial].avatar}</div>
              <div>
      ...
      <section style={{ padding: isMobile ? '52px 16px' : '88px 24px', background: '#f8fafc' }}>
        <div style={{ maxWidth: 780, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            ...
            <h2 style={{ fontSize: isMobile ? '26px' : 'clamp(28px, 4vw, 42px)', fontWeight: 800, color: '#0f172a' }}>Frequently Asked Questions</h2>
          </div>
            ...
            {faqs.map((faq, i) => (
              <div key={i} style={{ border: `1px solid ${openFaq===i?'#bfdbfe':'#e2e8f0'}`, borderRadius: 14, overflow: 'hidden', background: '#fff', boxShadow: openFaq===i?'0 4px 20px rgba(59,130,246,0.08)':'none' }}>
                <button onClick={() => setOpenFaq(openFaq===i?null:i)} style={{ width: '100%', padding: '16px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                  <span style={{ fontWeight: 600, fontSize: 13, color: openFaq===i?'#3b82f6':'#1e293b', lineHeight: 1.4 }}>{faq.q}</span>
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: openFaq===i?'#3b82f6':'#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginLeft: 12 }}>
                    <span style={{ color: openFaq===i?'#fff':'#64748b', fontSize: 16, lineHeight: 1, transform: openFaq===i?'rotate(45deg)':'none', display: 'block', transition: 'transform 0.2s' }}>+</span>
                  </div>
                </button>
                {openFaq===i && <div style={{ padding: '0 18px 16px', fontSize: 13, color: '#64748b', lineHeight: 1.75, borderTop: '1px solid #f1f5f9' }}>{faq.a}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section style={{ padding: isMobile ? '48px 16px' : '72px 24px', background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 40%, #3b82f6 70%, #6366f1 100%)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: isMobile ? 24 : 32, flexDirection: isMobile ? 'column' : 'row' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
              <div style={{ width: 56, height: 56, background: 'rgba(255,255,255,0.12)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, flexShrink: 0 }}>🚀</div>
              <div>
                <h3 style={{ color: '#fff', fontWeight: 800, fontSize: isMobile ? '20px' : '24px', marginBottom: 6, lineHeight: 1.3 }}>Start Your Digital Journey Today!</h3>
                <p style={{ color: 'rgba(255,255,255,0.72)', fontSize: 14 }}>Create your smart profile in 5 minutes. No technical skills needed.</p>
              </div>
            </div>
            <Link href="/dashboard/create-profile" style={{ background: '#fff', color: '#3b82f6', padding: '15px 32px', borderRadius: 12, fontSize: 15, fontWeight: 800, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, flexShrink: 0, boxShadow: '0 4px 20px rgba(0,0,0,0.2)', whiteSpace: 'nowrap' }}>
              Get Started Now
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
            </Link>
          </div>
