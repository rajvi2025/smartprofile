...
export async function GET(request) {
  try {
    ...
    // `to` is a date boundary the caller passes as the *start* of the day
    // after the last day they want included, so a plain `lt` gives an
    // inclusive end date without any off-by-one on same-day payments.
    ...
    // Attach business name for display — fetched separately since
    // Supabase JS doesn't guarantee a FK-based join is configured.
